...
import gui.display.newcase.NewCasePanel;
...
public class StartPanel extends JPanel implements ActionListener
{
	...
	public void actionPerformed(ActionEvent e)
	{
		...
		{
			this.manager.pushPanel(new NewCasePanel(this.manager), "PEMS - Create New Case");
		}
		else if (e.getSource() == this.editCaseButton)
		{
			try {
				this.manager.pushPanel(new EditCasePanel(this.manager), "PEMS - Edit Existing Case");
		...
		else if (e.getSource() == this.settingsButton)
		{
			...
			try {
				this.manager.pushPanel(new PrintSetUpPanel(this.manager, new ArrayList<Thumbnail>() ), "PEMS - PDF generator");
			} catch (IOException e1) {
				// TODO Auto-generated catch block
				e1.printStackTrace();
			}
